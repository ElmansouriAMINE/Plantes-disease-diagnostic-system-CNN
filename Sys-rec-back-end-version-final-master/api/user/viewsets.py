...
class UserViewSet(
    ...
    def getCommentaires_par_mois(self, request):
        ...
        commentaires_par_mois = commentaires.annotate(mois=ExtractMonth('date')).values('mois').annotate(commentaire_par_mois_count=Count('id')).order_by('mois')
    ...
    def getUsers_par_mois(self, request):
        users = User.objects.all()
        users_par_mois = users.annotate(mois=ExtractMonth('date')).values('mois').annotate(user_par_mois_count=Count('id')).order_by('mois')
    ...
    def count_predictions_by_type(self,count):
        types_list = ['Tomato___Late_blight', 'Tomato___healthy',   'Potato___healthy',  'Tomato___Early_blight', 'Tomato___Septoria_leaf_spot',  'Apple___Apple_scab', 'Tomato___Tomato_Yellow_Leaf_Curl_Virus', 'Tomato___Bacterial_spot', 'Apple___Black_rot',  'Apple___Cedar_apple_rust', 'Tomato___Target_Spot',  'Potato___Late_blight', 'Tomato___Tomato_mosaic_virus', 'Apple___healthy', 'Potato___Early_blight',    'Tomato___Leaf_Mold', 'Tomato___Spider_mites Two-spotted_spider_mite']
        types_count = [{'type': t, 'count': Prediction.objects.filter(classe=t).count()} for t in types_list]
        result = [{"type": item['type'], "count": item['count']} if item['count'] > 0 else {"type": item['type'], "count": 0} for item in types_count]
